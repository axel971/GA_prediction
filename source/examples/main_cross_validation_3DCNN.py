...
from model.model import CNN_3D
...
def main():
    ...
    # Perform cross-validation on the whole cohort ..
    nFold = 10
    foldSize = int(n_subject/nFold)
    
    for iFold in range(nFold):
    	
    	# Split the whole cohort in training and the validation data sets during the current fold
    	if iFold < nFold - 1 :
    	...
    	# Compute weights for the training samples using kernel density estimation
    	...
    	# Training of the 3D model
    	model = CNN_3D(image_size+[1])  		
    	...
    	model.save_weights('/home/axel/dev/fetus_GA_prediction/data/output/3DCNN/exp1/model_' + str(iFold) + ".h5")
    	
    	# Application of the trained model on the validation images (i.e., testing images)
    ...
    # Endpoint computation for the whole cohort
    ...
    np.save('/home/axel/dev/fetus_GA_prediction/data/output/3DCNN/exp1/GA.npy', GA)
    np.save('/home/axel/dev/fetus_GA_prediction/data/output/3DCNN/exp1/GA_prediction.npy', GA_predictions)
#     
